...
const AIImporterModal: React.FC<Omit<CalendarViewProps, 'allEvents' | 'onProfileUpdate'> & { onClose: () => void }> = ({ user, t, tSubject, getThemeClasses, showAppModal, userId, language, onClose }) => {
...
const CalendarView: React.FC<CalendarViewProps> = ({ allEvents, t, getThemeClasses, tSubject, language, showAppModal, userId, user, onProfileUpdate }) => {
    ...
    <div className="space-y-6 animate-fade-in">
        <div className="flex justify-between items-center flex-wrap gap-y-2">
            ...
            <div className="flex gap-2 items-center">
                 <button type="button" onClick={() => setIsSyncModalOpen(true)} className={`flex items-center text-white font-bold p-2 sm:py-2 sm:px-4 rounded-lg transition-transform active:scale-95 bg-sky-500 hover:bg-sky-600`}>
        ...
        <div className="flex items-center gap-2 p-1 bg-gray-200 rounded-lg">
            <button onClick={() => setViewMode('list')} className={`px-4 py-1.5 text-sm font-semibold rounded-md transition-colors w-full ${viewMode === 'list' ? `bg-white shadow text-gray-800` : `text-gray-500 hover:bg-gray-300`}`}>{t('today')}</button>
