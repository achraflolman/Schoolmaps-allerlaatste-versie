...
const NotificationsView: React.FC<NotificationsViewProps> = ({ user, notifications, t, getThemeClasses, setCurrentView, onProfileUpdate, showBroadcast, showAppModal }) => {
    ...
    const handleClearAll = () => {
        showAppModal({
            ...
            confirmAction: async () => {
                ...
                const feedbackIdsToDismiss: string[] = [];
                snapshot.docs.forEach(doc => {
                    const data = doc.data();
                    if (data.broadcastId) broadcastIdsToDismiss.push(data.broadcastId);
                    if (data.feedbackId) feedbackIdsToDismiss.push(data.feedbackId);
                });
